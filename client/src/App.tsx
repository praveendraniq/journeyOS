...
function TripOverview({ trip, setPage, activeDay, onReset }: { trip: Trip; setPage: (page: Page) => void; activeDay: number; onReset: () => void }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-[28px] border border-stone-200 bg-white p-6"><div className="flex items-end justify-between gap-3"><div><p className="eyebrow text-moss">Group preferences</p><h2 className="mt-1 text-xl font-bold text-ink">What Odyssey.AI knows so far.</h2></div><button onClick={() => setPage('planner')} className="text-xs font-bold text-moss">Manage friends</button></div><div className="mt-5 grid gap-3 md:grid-cols-3">{trip.travelers.slice(0, 3).map((traveler, index) => { const call = trip.preferenceCollection?.calls.find((item) => item.travelerId === traveler.id); const isPrabhu = isPrabhuProfile(traveler.id); const preloaded = index === 0 || isPrabhu; const status = preloaded ? 'Brief captured' : call?.status === 'completed' ? 'Call result captured' : call?.status === 'dialing' ? 'Call in progress' : 'Awaiting Friend 2 call'; const details = isPrabhu ? PRABHU_PROFILE.priorities.join(' · ') : index === 0 ? (call?.topPriorities.join(' · ') || `${traveler.pacePreference} pace · ${traveler.foodPreference}`) : call?.status === 'completed' ? call.summary : 'Friend 2’s preferences will appear here after the negotiation call.'; return <article key={traveler.id} className="rounded-2xl bg-[#fafbf9] p-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-full bg-ink text-[10px] font-bold text-white">{traveler.initials}</span><b className="text-sm text-ink">{traveler.name}</b></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${preloaded || call?.status === 'completed' ? 'bg-emerald-100 text-moss' : call?.status === 'dialing' ? 'bg-amber-100 text-amber-800' : 'bg-stone-100 text-stone-500'}`}>{status}</span></div><p className="mt-3 text-xs leading-5 text-stone-600">{details}</p>{isPrabhu && <p className="mt-3 text-xs font-semibold text-moss">{PRABHU_PROFILE.compromise}</p>}{call?.status === 'completed' && !preloaded && <p className="mt-3 text-xs font-semibold text-moss">{call.happiness}% plan fit · {call.compromise}</p>}</article>; })}</div></section>
...
function FriendSetup({ trip, onTrip }: { trip: Trip; onTrip: (trip: Trip, note: string) => void }) {
  ...
  return <section className="friend-setup rounded-[32px] border border-moss/20 bg-[#f6fbf7] p-6 xl:col-span-2">
    ...
    <div className="mt-5 grid gap-3 md:grid-cols-2">{drafts.map((friend, index) => {
      ...
      const isPrabhu = isPrabhuProfile(friend.id);
      const briefCaptured = isAdmin || isPrabhu;
      const outcome = isPrabhu ? { summary: PRABHU_PROFILE.summary, topPriorities: [...PRABHU_PROFILE.priorities], happiness: PRABHU_PROFILE.happiness, compromise: PRABHU_PROFILE.compromise, status: 'completed' as const } : call;
      return <article key={friend.id} className="rounded-2xl bg-white p-4 ring-1 ring-moss/10">
        ...
        {outcome && <PreferenceOutcome title={isPrabhu ? 'Preferences captured' : 'Call preference outcome'} summary={outcome.summary} priorities={outcome.topPriorities} fit={outcome.status === 'completed' ? `${outcome.happiness}% plan fit · ${outcome.compromise}` : undefined} />}
      </article>;
    })}</div>
