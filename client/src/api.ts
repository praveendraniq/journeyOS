...
export const api = {
  ...
  extractPlan: (conversation: string, trip?: Trip, structuredRequest?: Partial<TripRequest>) => request<{ request: TripRequest; source: string; confidence: number; summary: string; itinerarySource: 'google-places' | 'curated-fallback'; placesDiagnostic?: string; trip: Trip }>('/api/planner/extract', { method: 'POST', body: JSON.stringify({ conversation, trip, structuredRequest }) }),
  ...
  simulateMayaInterview: (trip: Trip) => request<{ trip: Trip; summary: string }>('/api/planner/simulate-maya-interview', { method: 'POST', body: JSON.stringify({ trip }) }),
  callMaya: (trip: Trip) => request<{ trip: Trip }>('/api/planner/call-maya', { method: 'POST', body: JSON.stringify({ trip }) }),
