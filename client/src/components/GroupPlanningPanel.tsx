...
export function GroupPlanningPanel({ trip, onTrip }: { trip: Trip; onTrip: (trip: Trip, note: string) => void }) {
  ...
  const collect = async () => {
    ...
    try {
      ...
      onTrip(response.trip, liveCalls ? `${response.collection.calls.length} outbound Vocal Bridge preference call${response.collection.calls.length === 1 ? '' : 's'} queued. The proposal will update after their call summaries are available.` : `${response.collection.calls.length} preference conversations completed. Odyssey negotiated a group proposal for admin review.`);
  ...
  return <><section className="mx-auto mt-4 max-w-[1400px] rounded-2xl border border-stone-200 bg-white px-5 py-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-bold text-ink">Friends & preference calls</p><p className="mt-1 text-xs text-stone-500">The trip admin stays on the plan. Remove any friend before collecting preferences.</p></div><div className="flex flex-wrap gap-2">{trip.travelers.slice(1).map((traveler) => <button key={traveler.id} disabled={busy} onClick={() => void remove(traveler)} className="rounded-xl border border-coral/25 bg-rose-50 px-3 py-2 text-xs font-bold text-coral disabled:opacity-40">Remove {traveler.name}</button>)}</div></div><label className="mt-4 flex cursor-pointer items-start gap-3 rounded-xl bg-[#f6fbf7] p-3 text-xs leading-5 text-stone-700 ring-1 ring-moss/10"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} className="mt-0.5 h-4 w-4 accent-[#245c4f]" /><span><b className="text-ink">Call consent confirmed.</b> I confirm each friend agreed to receive a short preference call for this trip.</span></label><button disabled={busy || !consent || !phoneReady || trip.travelers.length < 2} onClick={() => void collect()} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-moss px-4 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><Phone size={17} />{busy ? 'Calling friends…' : 'Call friends for preferences'}</button>{!consent && <p className="mt-2 text-center text-xs font-semibold text-stone-500">Confirm consent to enable calls.</p>}{!phoneReady && <p className="mt-2 text-center text-xs font-semibold text-coral">Add a usable phone number to every traveler before calling.</p>}{calls.length > 0 && <div className="mt-4 grid gap-3 md:grid-cols-2">{trip.travelers.slice(1).map((traveler) => { const call = callByTraveler.get(traveler.id); const callState = !call ? 'Not called' : call.status === 'completed' ? 'Preferences collected' : ['dialing', 'queued', 'connected'].includes(call.status) ? 'Call in progress' : call.status === 'no-answer' ? 'Call not attended' : call.status === 'failed' ? 'Call failed' : 'Call canceled'; return <article key={traveler.id} className="rounded-xl border border-stone-200 bg-stone-50 p-3"><div className="flex items-center justify-between gap-2"><b className="text-sm text-ink">{traveler.name}</b><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${call?.status === 'completed' ? 'bg-emerald-100 text-moss' : ['dialing', 'queued', 'connected'].includes(call?.status ?? '') ? 'bg-amber-100 text-amber-800' : 'bg-stone-200 text-stone-600'}`}>{callState}</span></div>{call?.status === 'completed' ? <div className="mt-3 rounded-lg bg-[#fff8e9] p-3"><p className="text-[10px] font-extrabold uppercase tracking-wider text-coral">Preference outcome</p><p className="mt-1 text-xs leading-5 text-stone-700">{call.summary}</p><p className="mt-2 text-xs font-semibold text-ink">Priorities: <span className="font-medium text-stone-600">{call.topPriorities.join(' · ')}</span></p><p className="mt-2 text-xs font-semibold text-moss">{call.happiness}% plan fit · {call.compromise}</p></div> : <p className="mt-2 text-xs leading-5 text-stone-600">{call?.summary ?? 'Consent is ready. Start the call when the group is ready.'}</p>}</article>; })}</div>}</section><div className="hidden">
